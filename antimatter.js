...
(() => {
  ...
  create_antimatter_crdt = (
    ...
    self.unsubscribe_cbs = self.unsubscribe_cbs || {};
    ...
    self.receive = (x) => {
      ...
      /// ``` js
      /// {type: 'unsubscribe', conn: 'CONN_ID'}
      /// ```
      if (type == "unsubscribe") {
        if (self.conns[conn] == null) throw Error("bad");
        send({ type: "ack", unsubscribe: true, conn });

        delete self.conns[conn];
        delete self.proto_conns[conn];
      }

      /// ## message unsubscribe `ack` 
      /// Sent in response to `unsubscribe`.. so they know we forgot them.
      /// ``` js
      /// {type: 'ack', unsubscribe: true, conn: 'CONN_ID'}
      /// ```
      if (type == "ack" && unsubscribe) {
        self.unsubscribe_cbs[conn]();
      }
    ...
    self.subscribe = (conn) => {
      ...
      send({ type: "subscribe", peer: self.id, conn, protocol_version: 2 });
    };

    /// # antimatter_crdt.unsubscribe(conn)
    ///
    ...
    ///
    /// ``` js
    /// alice_antimatter_crdt.unsubscribe('connection_to_bob')
    /// ```
    self.unsubscribe = async (conn) => {
      await new Promise((done) => {
        if (self.conns[conn] != null) {
          self.unsubscribe_cbs[conn] = done;
          send({ type: "unsubscribe", conn });
        }
